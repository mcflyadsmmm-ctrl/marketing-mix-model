 ...
 * Closed days through yesterday, clamped to the sales-fact window start
 * (Jan 1 of UTC year − 4 — same horizon as `salesDayFactWindowStartUtc`).
 * Pure — no I/O, no Shopify auth.
 */

/** Matches `SALES_DAY_FACT_WINDOW_YEARS_BACK` in sales-facts.server.ts. */
export const SPEND_TEMPLATE_WINDOW_YEARS_BACK = 4;
...
/**
 * UTC Jan 1 of (calendar year − 4), as YYYY-MM-DD.
...
export function spendTemplateDefaultFloorKey(now: Date = new Date()): string {
  const y = now.getUTCFullYear() - SPEND_TEMPLATE_WINDOW_YEARS_BACK;
  return `${y}-01-01`;
}
