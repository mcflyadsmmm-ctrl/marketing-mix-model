 ...
 * 2.1.1 plan picker loaded admin.shopify.com in the iframe.
 ...
 * the parser ignores the inner start tag, so billing CTAs would POST the outer
 * spend form instead of leaving the embed (pause email 2.1.1).
 */
...
describe("App Store resubmit path (email 2026-08-24 / ref 127166)", () => {
  it("Settings holds Start 7-day trial; product pages do not feature-gate Upgrade", () => {
    ...
    for (const rel of productPages) {
      ...
      expect(src, rel).not.toMatch(/ProUpgradeButton|ProUpsellBlock/);
  ...
  it("Partner testing paste pack covers 4.5.4 checkbox + 2.1.1 plan picker _top", () => {
  ...
  it("4.5.4 testing instructions include credentials (none) and never a password placeholder", () => {
    ...
    expect(paste).toMatch(/Live data/i);
    expect(paste).not.toMatch(/Upgrade to Pro for LTV/);
    expect(paste).not.toMatch(/<PASTE/);
    expect(listing).not.toMatch(/<PASTE CURRENT STAFF PASSWORD>/);
  });

  it("Spend Add spend Form does not nest billing CTAs (2.1.1 nested-form brick)", () => {
  ...
  it("route billing CTAs are never nested inside a parent Form", () => {
    ...
    for (const name of files) {
      ...
      for (const depth of depths) {
        expect(depth, `${name} nested billing CTA`).toBe(0);
      }
    }
  });
