...
| --- | --- | --- |
| 2.1.1 Admin in iframe | `billing-exit.server.ts` HTML + `window.open(_, "_top")`; POST + client navigate; never bare 302 to Admin | Deployed `ProUpgradeButton-BeJLYLQ_.js` contains `openTop` / `embedded=1` / Admin same-frame refuse |
| 4.5.4 / 4.5.5 password | Paste pack in [`APP_STORE_LISTING.md`](./APP_STORE_LISTING.md) | **HUMAN** must paste real staff password into Partner |
...
1. Partner → testing instructions → paste **real** staff password for `mcflyadsmmm@gmail.com` on `devmcflyads.myshopify.com`
