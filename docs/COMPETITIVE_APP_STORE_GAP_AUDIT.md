...
| --- | --- | --- | --- |
| 1 | **Live trust URLs lag** — was invite-only / soft-launch vs Free listing | Reviewer trust mismatch | **CLOSED** (Pages Free closed 2026-07-26; re-curl before Submit) |
...
| 4 | **Homepage primary CTA still waitlist / early access** while listing draft = Free App Store | After listing goes live, site→listing journey feels invite-only → bounce / “is this real?” | **CLOSED** (`site/index.html` Install free → `/support`; CTA config) |
| 5 | **Brand dual-name** — site “Mcfly Ads” vs listing “Mcfly Analytics” | Trust friction for reviewers & merchants comparing Privacy/Support titles to app name | **CLOSED** (trust titles Mcfly Analytics; pricing title aligned 2026-07-30; Ads lockup OK in chrome) |
...
| 8 | **Stale reject-audit rows** still claim support meta “Invite-only” | Local `site/support.html` meta already Free App Store — audit noise steers agents into circles | **CLOSED** ([`REJECT_RISK_AUDIT.md`](./REJECT_RISK_AUDIT.md) 2026-07-30 refresh) |
...
**Approval verdict (competitive lens):** Mcfly is **not blocked by missing TW features**. It **is** blocked by human gates (shots + Partner/Submit) — Pages Free lag closed; remaining **listing/site honesty** AGENT_FIX items should keep Free + PCD as one story.
...
| --- | --- | --- | --- |
| 1 | **Close stale “Invite-only meta” rows** — local support meta is already Free App Store | `docs/REJECT_RISK_AUDIT.md` risk #8 / P2 row; `docs/SUBMIT_TOMORROW.md` open-agent table | **DONE** 2026-07-30 |
| 2 | **Homepage CTA path for App Store Free** — lead with find/install narrative (or dual: App Store + design-partner email); never shop-domain form | `site/index.html` hero CTA + `#waitlist` band; optional `site/download.html` | **DONE** — Install free |
| 3 | **Partner listing completeness checklist** — Languages=English; Works with=**blank** (honest; no fake Meta); Category pick; developer display name; Free pricing; no External charges while Free | `docs/APP_STORE_LISTING.md` §Partner fields | **DONE** in listing SoT |
| 4 | **First-10-minutes TTFV block** in long description / reviewer notes — Settings margin → CSV (incl. Other) → Cash MER → Allocation | `docs/APP_STORE_LISTING.md` long description + reviewer notes | **DONE** in listing SoT |
| 5 | **Differentiate listing shot 2** — not just period swap of shot 1; caption must prove formula, not “another scoreboard” | `docs/LISTING_VISUAL_PACK.md` shot table; optionally note UI affordance already present | **DONE** in pack (Explorer sales÷spend) |
| 6 | **Trust-page name alignment** — titles/H1 use **Mcfly Analytics** (app name) with Ads lockup OK in chrome | `site/support.html`, `pricing.html`, `privacy.html`, `terms.html` | **DONE** (pricing title 2026-07-30) |
| 7 | **Extensionless canonicals** on trust pages (308 already; prefer extensionless in meta) | same trust HTML `link rel=canonical` + `og:url` | **DONE** |
| 8 | **Support FAQ: “Why no Meta/Google connect?”** — CSV-first honesty (religion) so week-1 expectations don’t churn | `site/support.html` FAQ section | **DONE** (“Do I connect Meta or Google?”) |
| 9 | **Live-vs-local trust regression note** for agents — curl recipe that fails if invite-only returns | `docs/REJECT_RISK_AUDIT.md` or `scripts/` one-liner in ship notes | **DONE** |
| 10 | **Icon + shot upload runbook one-pager cross-links** — filenames, crop, sample OFF, M-only icon path | `docs/LISTING_VISUAL_PACK.md` ↔ `docs/SUBMIT_NOW.md` · [`ops/SUBMIT_HANDOFF.md`](./ops/SUBMIT_HANDOFF.md) | **DONE** |
...
| Trust URLs match listing | Always | **Pages Free closed** (re-curl before Submit) | Spot-check |
...
- Live curl: `mcflyads.com/support|pricing|privacy` — **Pages Free lag closed** (2026-07-26); re-verify Free + PCD copy before Submit; Fly `/health` ok  
