...
**Pricing:** Shopify App Pricing — **one plan**: 7-day trial, then **$39/store/mo** for the whole desk. **Product:** depth Shopify Analytics skips first—Overview YoY (this month / quarter / year vs last year), typical order, Customers / Orders / LTV (Growth and LTV are Customers chips). Spend is optional typed/CSV plus certified Total ROAS chips (honest MER: sales ÷ entered spend). Empty spend is an em dash, not 0×. Never pixels / MTA / “true ROAS” / Profit Agent / P&L as hero. **Trial is 90 days of order history. Paid is up to 24 months.** SAMPLE is on https://mcflyads.com/demo and Fly `/demo`. Admin is this shop’s Live book when unparked.
...
**Status (2026-09-22):** Listing is **live** at https://apps.shopify.com/mcfly-analytics-public (handle `mcfly-analytics-public`, **reviews: 0**). Public listing copy is still spend-led (“See every ad dollar next to Sales”) and still has **$39 in a feature bullet** (4.2.3). Tagline, short, long, and feature bullets below are the **sales-first paste Marty should Save** (Shopify five first at zero spend; optional Total ROAS = honest MER; **trial 90 days / paid 24 months**; Live Admin when unparked + SAMPLE on `/demo`). Matches Fly **v433** desk + Fly-served `site/`. **mcflyads.com Pages is stale** (24 months on trial / full-access) until Mac Direct Upload. Cursor does not Submit. **4.2.2 / 4.2.3:** merchant-facing listing paste (short / long / features / captions / images) must **not** include plan prices. Put **$39** only in Partner **Pricing details** and in reviewer testing notes. One plan covers the whole desk — five analysis tabs plus Settings. Nothing is feature-gated. **Website** = `https://mcflyads.com` **after Pages matches Fly**. **App URL / Privacy / Support / Terms** stay on Fly `https://mcfly-analytics.fly.dev`. Never App URL = mcflyads.com. Human paste pack: [`ops/LISTING_LIVE_PASTE.md`](./ops/LISTING_LIVE_PASTE.md).
...
- [ ] **Long description** opens on Overview YoY and Shopify-depth tabs; Total ROAS is chapter two as honest MER; **trial 90 days / paid 24 months**; SAMPLE on `/demo`; **no $ /mo in this field**
- [ ] **Sales channel requirements:** do **not** check “Merchant must have online store” — this app is Admin-only (4.3.1)
- [ ] **Trust URLs** stay on Fly: `https://mcfly-analytics.fly.dev/privacy` `/support` `/terms`. Never App URL = mcflyads.com. **Website** = `https://mcflyads.com` **after Pages matches Fly** (trial **90 days** / paid **24 months**). Do not Submit while Pages still says 24 months on trial.
...
Partner listing URLs (live listing `mcfly-analytics-public`). **Privacy / Support / Terms / App URL** stay on Fly. Partner **Website** field is `https://mcflyads.com` only after Pages matches Fly. Never App URL = mcflyads.com.
...
| `read_all_orders` (deep history) | **In live TOML / Fly SCOPES** (Partner-approved for history depth). Unpaid/trial ingest is still **90 closed days**. Paid order rows cap at 24 months. Reviewer notes must not say trial includes 24 months. Still **no** Level 2 PII fields. |
...
| **Website** | https://mcflyads.com | Only after Pages matches Fly (90 days on trial). Privacy / Support / Terms stay Fly. |
...
2. Overview → this month / quarter / year vs last year, typical order, chart.
   Trial stores 90 days of order history. Paid stores go up to 24 months.
   Missing last year stays an em dash — never invent last year as zero.
3. Open Orders and Customers (Growth and LTV are Customers chips). Then Goals.
   Those pages work at zero spend.
4. Admin is this shop’s Live book when unparked. Labeled SAMPLE walkthrough is
   on https://mcflyads.com/demo — not inside Admin. There is no Sample | Live
   toggle in Settings.
   Optional: Spend → type one daily spend entry or upload a spend file.
   CSV remains for many days / Ads Manager export.
   Optional automation: Spend → Mcfly template → SyncWith / Coupler /
   Supermetrics / Coefficient (you pay those tools) → CSV → Paste / Import.
   Not a “Works with” partnership.
   Tabs are not feature-gated. Trial ingest is 90 closed days. Paid is 24 months.
5. Total ROAS → honest MER (Shopify Total Sales ÷ spend you entered). Certified
   ...
   Settings target. Empty spend is an em dash, not 0×. Mix, CPA, ledger, and
   dual-close sit on Spend.
```

**Listing honesty:** Default plan = every named platform plus extras like billboards (type or CSV). **Optional automation** = merchant-paid SyncWith-class tools filling a Mcfly template (no fake Works-with). Trial and paid both get the five analysis tabs plus Settings — nothing is feature-gated. Trial order rows are 90 closed days; paid is up to 24 months. We do not require a pixel. We do not claim attributed campaign ROAS, Profit Agent, or a P&L suite. We do not replace Ads Manager numbers.
...
**Feature Image 1 (header media):** **Marty recapture** Live Admin Overview YoY (Fly **v433** or current, Live unparked) and overwrite `docs/listing-assets/feature-media-1600x900.png`. Show this month / quarter / year vs last year, typical order, and the sales chart. Admin is this shop’s Live book when unparked — do not use https://mcflyads.com/demo SAMPLE for the live listing hero. Do not upload the old formula-only / three-year Total ROAS image or sample ROAS stats (`4.42x`, `Above break-even 2.86x`). Shots were **not** recaptured for this pack.
...
=== HOW WE USE read_all_orders ===
Partner-approved. Lets the paid book store up to 24 months of order rows (opaque customerKey + amounts/dates only). Unpaid / trial ingest is still 90 closed days. Still Level 1 — never Level 2 PII fields.
...
| Till LTV? | Opaque id + order amounts/dates → CohortFact (30/90/365). Still Level 1. `read_all_orders` is in live TOML / Fly. Unpaid / trial ingest is 90 closed days. Paid order rows cap at 24 months. Not Level 2. |
...
- [ ] Publish Cloudflare Pages from honest `site/` so mcflyads.com `/` `/pricing` match Fly (trial **90 days** / paid **24 months**). Fly `/privacy` `/support` `/terms` are already honest. **Do not Submit while Pages still says 24 months on trial.**
- [ ] **Live unpark** per [`ops/LIVE_UNPARK_CHECKLIST.md`](./ops/LIVE_UNPARK_CHECKLIST.md) so App Review sees this shop, not SAMPLE (**human**)
- [ ] Install on `devmcflyads`; smoke test above; SAMPLE chip **off** on the review shop (**human**)
