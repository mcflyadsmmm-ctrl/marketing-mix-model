...
**Retired.** Do **not** paste Free Meta+Google / Pro unlocks / Free+Pro matrix / **(paid)** feature-bullet suffixes into Partner short, long, features, captions, or Pricing.
...
| Feature bullets with **(paid)** on LTV / Goals | **Live residue (2026-09-08).** Strip → paste clean bullets in [`ops/LISTING_LIVE_PASTE.md`](./ops/LISTING_LIVE_PASTE.md) §5 |
...
## Explicitly deferred (do not expand scopes)
...
**Pre-flight:** Distribution = **Shopify App Store** · PCD submitted · trust URLs still $39 + PCD on live curl ([`REJECT_RISK_AUDIT.md`](./REJECT_RISK_AUDIT.md)).
...
```text
FIRST 10 MINUTES (no pixels; $39 desk = all named channels including TikTok CSV)
...
Demo SAMPLE desk is preview data only — turn OFF before judging live metrics.
```

**Listing honesty:** One $39 plan (7-day trial) = all named channels, Customer LTV / Acquisition, full-year Goals board. **Optional automation** = merchant-paid SyncWith-class tools filling a Mcfly template (no fake Works-with). SAMPLE is preview data only. Still refuse connector zoo / pixels. Coexists with attribution suites.
...
| 3 | Spend CSV | Meta + Google + TikTok → paste / combine | Named channels + export guides + combine import |
...
=== HOW WE USE read_all_orders (live in scopes) ===
Deep till history beyond ~60 days for OrderFact backfill and till LTV cohorts (opaque customerKey + amounts/dates only). Still Level 1 — never Level 2 PII fields. Declared in shopify.app.toml and Partner-approved. Without the merchant grant, history is limited and cohorts underclaim.
...
| Till LTV? | Opaque id + order amounts/dates → CohortFact (30/90/365). Still Level 1. `read_all_orders` is live in scopes and unlocks history beyond ~60 days (paste block in [`PCD_AND_LTV.md`](./PCD_AND_LTV.md)). Not Level 2. |
| CSV upload contents? | Merchant-supplied **ad-spend aggregates only**. Wide: Day + Meta/Google/Microsoft/TikTok/…/named platforms. Long `date,channel,amount` also works. Sales columns ignored. No customer data. Optional SyncWith-class tools are merchant processors. |
...
SMOKE TEST (CSV-first $39 desk = all named channels)
...
5. Spend Allocation → quarters / pie / rolling windows when spend > 0
...
- read_all_orders: **live in scopes** — deep till history for OrderFact / till LTV cohorts (no PII)
...
read_all_orders is Partner-approved and declared in shopify.app.toml. If a token
lacks it, the merchant sees a grant CTA — history is not permanently dead.
```
