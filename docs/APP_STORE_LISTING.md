...
**Listing honesty:** Default plan = every named platform plus extras like billboards (type or CSV). **Optional automation** = merchant-paid SyncWith-class tools filling a Mcfly template (no fake Works-with). Trial and paid both get the eleven analysis tabs plus Settings — nothing is feature-gated. We do not require a pixel. We do not claim attributed campaign ROAS, Profit Agent, or a P&L suite. We do not replace Ads Manager numbers.
