...
**Base this plan is cut from:** `origin/cursor/spend-trust-recurring` at `d2017c1` (`docs: stamp Fly v423 and name whale ticket`). The five notes scored that same tip. Fly **v423** / merge `8938134` (`#172` quiet-back dollars) is on the tip. Whale `#177` / `fad2924` is in review — the named current craft, not a find. Quiet-back is not listed as missing.
...
## Named next ship after whale (SCOREBOARD stamp)

After `#177` Reviewer PASS + merge + Fly **v424**, stamp the scoreboard with **this queue’s rank 1** only:
...
Branch `cursor/buyer-life-honesty-5bc6`. One implementer. One Reviewer. One Fly only after PASS. Do not fly this plan. Do not fly a SoT stamp.
...
- Do not recook v408–v423, `#172` quiet-back, or `#177` whale.
...
**Named current craft, not this queue:** whale ticket / RFM flow / watchlist “N more” (`cursor/whale-ticket-5bc6`, `#177`, `fad2924`). Typical ticket on the row. Cold share in the existing Slack line. Of last month’s Champions, how many are At risk or Hibernating now. Do not restart it. Do not list it as a find.

## Rank 1 — next cook (after whale PASS + Fly v424)
...
**Branch:** `cursor/buyer-life-honesty-5bc6`, cut from `origin/cursor/spend-trust-recurring` after whale merge (or Marty’s skip). One implementer. One Reviewer. One Fly only after PASS.
...
**Wait:** whale (`#177`). `customers-analytics.ts` is then this writer’s until rank 12 / 14.
