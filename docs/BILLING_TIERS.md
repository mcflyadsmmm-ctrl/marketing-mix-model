...
**Live ingest depth (not a feature gate):** trial and paid share the same desk. Order rows stop at 24 months for both. Billing does not withhold history until the first charge. PCD Level 2 is Approved. `read_reports` is shipped so Live can call ShopifyQL for aggregate day totals. We still do not store name, email, phone, or address. SAMPLE does not paint those QL clocks. Production (2026-09-23) is unparked to `overview_orders` via Fly secrets (`MCFLY_SAMPLE_ONLY=false`). The SAMPLE toggle still exists. Customers and LTV stages are not unlocked. Git `fly.toml` `[env]` stays the kill-switch default (`true` / `parked`); secrets override production and Marty re-asserts them after deploy. SAMPLE stays full wow. Price does not rise with sales. A host with billing off uses the paid window.
