...
- **Closed 2026-09-10 (Love-8 → Track D2):** the gap and the recon lock above are historical. The page now leads with the call: `resolveAllocationPlan` (`app/app/lib/allocation-recommendation.ts`) spends mer-core's already-floored percent against period cash — "Cut Portfolio spend by $350 (~35%) for 7 days · keep $650 of $1,000 live, floor $500 (50% of period spend)" — or "Keep all $1,000" above break-even, with the computed hold / reduce / step-test steps under it. Locks come from one resolver (`app/app/lib/allocation-lock.ts`) with a named reason: no spend, margin unset, coverage under 70% plus the missing-day count, sales-load error, untrusted $0 sales. Declared-vs-desk recon is no longer a lock (Love-7 deleted the unreachable read).
...
2. **Allocation over-promises its own render.** Empty state: "then this page says which channels to cut or keep." The page shows quarters, a pie, and rolling windows. Either render the computed `actions` or change the copy. *Closed 2026-09-10 — the call renders in dollars when trusted, and the copy no longer promises channel-level cut/keep; see §2.5.*
