...
| Field | Value |
| --- | --- |
| **Updated** | 2026-09-25 · America/Denver · Site probe **v5/visual-site** (drift — Pages republish pending) · occupancy **0/4** · Wave B QL fold **shipped** `279a5c6` · ads **NO** · reviews **0** · **#221 MERGED** whole-desk default on tip · Fly **v475+** · Conductor final draft executing · **Marty: Admin Result only** (secrets already `false`/`ltv`) |
| **Live** | https://mcflyads.com · probe `mcfly-version` **v5** / build visual-site (board previously v45 — do not invent) · H1 TBD on republish |
| **Fly** | https://mcfly-analytics.fly.dev · tip `279a5c6` Overview QL fold · `/health` **200** · git `[env]` kill-switch `true`/`parked` · SSH assert PASS: `MCFLY_SAMPLE_ONLY=false` · `MCFLY_LIVE_STAGE=ltv` · Fly **v477** |
...
Restraint. Paper/sky. Original ribbon M. Phone = brand + hamburger.

**Spine:** `/` → Install (App Store) · `/demo` still on nav  
**Nav ≤4:** Demo · Pricing · About · (CTA **Install**)
...
| Tier | Rule |
| --- | --- |
| **A** | `/` `/demo` `/pricing` `/about` `/product` chrome — one brand |
| **B** | `/privacy` `/support` `/terms` — 200, heads on mcflyads.com |
| **C** | Custom + suite landers + `/lab` + `/cash-mer` + `/monday-close` | **301 /** |
| **D** | `app/**` + Fly | Unfrozen — approval: OAuth, `/app`, trust URLs, $39/7-day |
...
- [x] v11 app-first
- [x] **v12 live** — one brand · Pages `b9a0268a`
...
| v11 | `ebe77a79` | App-first site · Custom parked · Harbor SAMPLE |
| v12 | `b9a0268a` | One brand: Analytics titles/favicons/OG · Custom 301 home |
