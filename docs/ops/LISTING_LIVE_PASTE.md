# Listing live paste pack — Mcfly Analytics
...
Work top → bottom. **Save each section.** Do **not** click Submit.

1. **Apps** → **Mcfly Analytics Public** (`403721814017`).
2. **Distribution** → Shopify App Store listing → **Manage listing** / **Edit listing**.
3. **Pricing details** first (billing honesty): one plan named **Mcfly Analytics**, **$39 USD / 30 days**, **7-day free trial**. Rename **Pro** → **Mcfly Analytics**. **Delete any Free plan.** Confirm the picker shows **one** plan only. See §7.
4. **Listing basics:** App name `Mcfly Analytics` · language English · category Marketing → Marketing analytics (or Advertising). **Works with = blank.** **Do not** check “Merchant must have online store” (4.3.1).
5. **Tagline** → paste §2.
...
10. **URLs**
    ...
    | **Website** | `https://mcflyads.com` | Spot-check the live mcflyads.com version before Save (home / `/demo` / pricing). Keep Website = `https://mcflyads.com`. App URL stays Fly. |
...
**History check before Save:** long description and reviewer notes say trial and paid share the full desk and up to **24 months** of orders — not a 90-day trial book, not “~60 days.” Empty YoY cells stay honest (never last year as zero); do not invent GMV or reviews.
...
Pick one. All three marked **Marty picks**.

```text
Option 1 (Marty picks): Shopify Analytics skips YoY, typical order, returning dollars, and LTV

Option 2 (Marty picks): This month vs last year, typical order, returning dollars — from orders

Option 3 (Marty picks): Your Shopify orders as a morning number. Spend next to sales, flat $39
```
...
## 4. Long description (`APP_STORE_PASTE:long`)

Paste the block below. **App URL / Privacy / Support / Terms stay Fly** — never App URL = `mcflyads.com`.

Set Partner **Website** and the long-description **Learn more** line to `https://mcflyads.com`. Spot-check the live mcflyads.com version before Save. Privacy / Support / Terms / App URL stay on Fly (`https://mcfly-analytics.fly.dev`).
...
Customers, Orders, and Overview work with no spend: days to a second order, weekend share, and new-buyer value at 30 / 90 / 365 days live on those pages. Trial and paid share the full desk and up to 24 months of orders. Admin opens on your live store orders. Never invent last year as zero while history is still loading. Try the labeled SAMPLE walkthrough anytime at https://mcflyads.com/demo — SAMPLE is not inside Admin.
...
Privacy / Support / App URL lines in that block stay on `https://mcfly-analytics.fly.dev`.

No `$` / ` /mo` in this field (4.2.3). No “Free” / “Pro” / “one plan” in this field.

---

## 5. Feature bullets (`APP_STORE_PASTE:features` — paste in order)
...
Paste exactly (comma-separated if Partner wants a single field):
...
1. Rename the paid plan from **Pro** to **Mcfly Analytics**.
...
**Marty must verify in Partner Pricing** that there is **no Free plan**. [`BILLING_TIERS.md`](../BILLING_TIERS.md) warned the live managed-pricing page still showed **Free + Pro $39** as of **2026-08-26**. Agents cannot see Partner Pricing from git.
...
3. Crop ~1600×900 of the APP BODY only.
   ...
   Exclude: Admin left nav, OS menubar, URL bar, and Total ROAS chips (those are shot 5).
4. Overwrite (do not create a new filename):
...
| File | Why |
| --- | --- |
| `docs/listing-assets/shots/04-free-pro-pricing.png` | Plan prices in the image — 4.2.2 |
...
**Marty recapture all five screenshots from Live Admin** (current production Fly). Do not use Snowdevil `/demo` SAMPLE for App Store stills. Founder-pack and older Fly files do not match the ready desk. Overwrite the stable filenames; do not invent new files. Do **not** invent that these shots were already recaptured.
...
## 10. Optional handle email (Partner Support)

Live URL stays `https://apps.shopify.com/mcfly-analytics-public` until Shopify changes the handle. Do not invent a cleaner listing URL.

Preference order: **`mcfly-analytics`** / **`mcfly`** / **`mcfly-spend`**.

```text
Subject: Request listing handle change — Mcfly Analytics (app 403721814017)

Hi Shopify Partner Support,

Please change the App Store handle for Mcfly Analytics (app ID 403721814017), currently mcfly-analytics-public, to one of these in preference order:
...
| --- | --- | --- |
| 1 | `01-total-roas-vs-breakeven.png` | See this month, quarter, and year vs last year plus typical order |
...
- Website / Learn more → `https://mcflyads.com`. Spot-check the live mcflyads.com version before Save. App URL stays Fly.
- **History check:** long copy says trial and paid share the full desk and up to **24 months** of orders — not a 90-day trial book, not ~60d as primary
...
## Listing copy that still contradicts one-plan billing

Public paste blocks in this pack (tagline / short / long / features / captions) do **not** say Free / Pro / $39. These **other** lines still imply a Free gate or a second plan — do not paste them into the live listing body:

| Where | What |
| --- | --- |
| Live Partner **plan name `Pro`** | Implies a Free tier. Rename to **Mcfly Analytics**. |
| Live Partner **Free plan** (if still present; warned 2026-08-26) | Delete it. One plan $39 + 7-day trial. |
| Live listing **feature bullet with $39** | 4.2.3. Replace with §5. |
| Founder PNG `04-free-pro-pricing.png` | Prices in the image. Do not upload. |
| Founder PNG `05-spend-csv.png` | “Other platforms on Pro.” Do not upload. |

`$39` belongs only in Partner **Pricing details** (and reviewer testing notes), never in short / long / features / captions / PNGs.
